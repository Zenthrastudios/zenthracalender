...
export default function CourseViewer() {
    ...
    useEffect(() => {
        ...
        lessons.forEach(lesson => {
            ...
            vid.muted = true;
            // Do NOT set crossOrigin here — without it, iOS/Safari loads metadata fine.
            // Canvas drawImage will fail for cross-origin videos (CORS taint) but
            // we catch that below. Duration loading still works without CORS attr.
            ...
            vid.onloadedmetadata = () => {
                ...
                // Only attempt canvas thumbnail on non-iOS (canvas is tainted on iOS for cross-origin)
                if (!isIOS) {
                    vid.currentTime = 0.1;
                } else {
                    // On iOS: skip canvas, just clean up
                    vid.src = '';
                }
            };
            ...
            vid.onseeked = () => {
                try {
                    ...
                    // CORS or canvas security error — thumbnail unavailable, EP N placeholder shown
                }
                vid.src = '';
            };
