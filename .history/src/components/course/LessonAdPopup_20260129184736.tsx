...
export default function LessonAdPopup({ isOpen, onClose, settings, lessonId }: LessonAdPopupProps) {
        ...
        async function fetchDetails() {
            ...
            if (settings.type === 'booking' && settings.booking_event_type_id) {
                ...
                const { data: productData } = await supabase
                    ...
                    .select('title, slug, user_id, image_url')
                    .eq('id', settings.product_id)
                    .single();

                if (productData) {
                    setFetchedTitle(productData.title);
                    if (productData.image_url) setFetchedImage(productData.image_url);
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="sm:max-w-md p-0 overflow-hidden bg-zinc-950 border-zinc-800 text-white gap-0">
