...
-- Add schedule_id to availability table (if not exists)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'availability' AND column_name = 'schedule_id') THEN
    ALTER TABLE public.availability ADD COLUMN schedule_id UUID REFERENCES public.availability_schedules(id) ON DELETE CASCADE;
  END IF;
END $$;

-- Add schedule_id to event_types table (if not exists)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'event_types' AND column_name = 'schedule_id') THEN
    ALTER TABLE public.event_types ADD COLUMN schedule_id UUID REFERENCES public.availability_schedules(id) ON DELETE SET NULL;
  END IF;
END $$;
...
-- Policies for availability_schedules (drop if exists, then create)
...
DROP POLICY IF EXISTS "Users can manage their own schedules" ON public.availability_schedules;
CREATE POLICY "Users can manage their own schedules" ON public.availability_schedules
  FOR ALL USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Public can view schedules for booking" ON public.availability_schedules;
CREATE POLICY "Public can view schedules for booking" ON public.availability_schedules
  FOR SELECT USING (true);

-- Create trigger for updated_at (drop if exists)
...
CREATE INDEX IF NOT EXISTS idx_availability_schedules_user_id ON public.availability_schedules(user_id);
CREATE INDEX IF NOT EXISTS idx_availability_schedule_id ON public.availability(schedule_id);
CREATE INDEX IF NOT EXISTS idx_event_types_schedule_id ON public.event_types(schedule_id);
