...
function CanvasLogic({
  ...
  videoRef: any;
  isPlaying: boolean;
  setSecurityWarning: (w: boolean) => void;
}) {
      ...
      mutations.forEach((mutation) => {
        ...
        mutation.addedNodes.forEach((node) => {
          if (node instanceof HTMLElement) {
            ...
            if (
              className.includes("loom") ||
              className.includes("recorder") ||
              className.includes("screenity")
            ) {
              found = true;
            }
          }
...
export default function CourseViewer() {
  ...
  const iosDurationHydratedRef = useRef<Set<string>>(new Set());
  const [lessonThumbnails, setLessonThumbnails] = useState<
  ...
  const SUPABASE_FN_URL =
    "https://zlhbzlxxdezlrtzljpni.supabase.co/functions/v1/video-access";
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", preventScreenshotKeys, {
      capture: true,
    });
    ...
    return () => {
      window.removeEventListener("keydown", preventScreenshotKeys, {
        capture: true,
      });
  ...
  useEffect(() => {
    ...
    const thumbnailUrl = currentLesson?.id
      ? lessonThumbnails[currentLesson.id]
      : null;
  ...
  useEffect(() => {
    async function loadCourse() {
      ...
      try {
        // Use edge function to bypass RLS for public access lookup
        const { data: result, error: fnError } =
          await supabase.functions.invoke("recover-course-access", {
            body: { token: accessToken },
          });

        if (fnError || !result?.found) {
          throw new Error(
            result?.error ||
              fnError?.message ||
        ...
        const purchaseData = result.purchase;
        ...
        // Get lessons (course_lessons is public, but still use db for consistency)
      ...
      const generateThumbnail = () => {
        ...
        vid.onloadedmetadata = () => {
          ...
          if (dur > 0) {
            setLessons((prev) =>
              prev.map((l) =>
                l.id === lesson.id ? { ...l, video_duration: dur } : l
              )
            );
          }
        ...
        vid.onseeked = () => {
          try {
            ...
            console.warn(
              `Failed to generate thumbnail for lesson ${lesson.id}:`,
              error
            );
        ...
        const fallbackThumbnail = () => {
          ...
          fallbackVid.onseeked = () => {
            try {
              ...
              console.warn(
                `Final thumbnail generation failed for lesson ${lesson.id}`
              );
            }
            fallbackVid.src = "";
          };
  ...
  // iOS-safe duration hydration: load one missing duration at a time, only while not playing.
  // No seeking/canvas work here, just metadata read to avoid stressing Safari's media pipeline.
  useEffect(() => {
    if (!isIOS || !accessToken || isPlaying || lessons.length === 0) return;
    ...
    const targetLesson = lessons.find(
      (lesson) =>
        ...
        !iosDurationHydratedRef.current.has(lesson.id)
    );

    if (!targetLesson) return;
    iosDurationHydratedRef.current.add(targetLesson.id);
    ...
    const streamUrl = `${SUPABASE_FN_URL}?lessonId=${targetLesson.id}&token=${accessToken}`;
    let disposed = false;
    ...
    probe.onloadedmetadata = () => {
      if (disposed) return;
      const durationSeconds = Math.floor(probe.duration);
      if (durationSeconds > 0) {
        setLessons((prev) =>
          prev.map((lesson) =>
            lesson.id === targetLesson.id
              ? { ...lesson, video_duration: durationSeconds }
              : lesson
          )
        );
      }
  ...
  const updateProgress = useCallback(
    async (lessonId: string, seconds: number, isCompleted: boolean) => {
      ...
      await db
        .from("course_progress")
        .upsert(progressData, { onConflict: "purchase_id,lesson_id" });
  ...
  const handleLoadedMetadata = useCallback(() => {
    if (videoRef.current) {
      ...
      if (
        ...
        if (
          currentLesson &&
          (currentLesson.video_duration === 0 || !currentLesson.video_duration)
        ) {
          setLessons((prev) =>
            prev.map((l) =>
              l.id === currentLesson.id
                ? { ...l, video_duration: Math.floor(vidDuration) }
                : l
            )
          );
        }
        ...
        if (lessonProgress && lessonProgress.progress_seconds > 0) {
          ...
          console.log(
            "▶️ Resumed from progress:",
            lessonProgress.progress_seconds
          );
        }
      } else {
  ...
  const getLessonDurationSeconds = (lesson: LessonData) => {
    if (lesson.video_duration && lesson.video_duration > 0)
      return lesson.video_duration;
    if (lesson.id === currentLesson?.id && duration > 0)
  ...
  const displayDuration =
    duration > 0
      ? duration
      : currentLesson?.video_duration && currentLesson.video_duration > 0
      ? currentLesson.video_duration
      : 0;

  const getLessonThumbnailSrc = (lessonId: string) => {
    return lessonThumbnails[lessonId] || purchase?.course.thumbnail_url || null;
  };
  ...
  const overallProgress = useMemo(() => {
    ...
    const currentTotalProgress = lessons.reduce((sum, lesson) => {
      ...
      const midwayPercent = Math.min(
        99,
        (lp.progress_seconds / lessonDuration) * 100
      );
      ...
      <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-4">
        ...
        <div className="relative z-10 w-full max-w-md">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 text-lg font-bold text-white mb-2">
              <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center">
                <GraduationCap className="w-4 h-4 text-white" />
              </div>
              Intimate<span className="text-primary">care</span>
            </div>
          </div>
          ...
          <div className="bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl">
            ...
            <div className="p-8">
              ...
              <div className="flex justify-center mb-6">
                <div className="w-20 h-20 bg-primary/15 rounded-full flex items-center justify-center border border-primary/30">
              ...
              <p className="text-zinc-400 text-sm text-center mb-2">
                Enter the email you used when purchasing this course and we'll
                send your access link right away.
              </p>
              ...
              {/* Recovery Form */}
              <form onSubmit={handleAccessRecovery} className="space-y-4">
              ...
              <div className="mt-6 pt-6 border-t border-zinc-800">
                ...
                <button
                  ...
                  className="w-full h-10 rounded-xl border border-zinc-700 text-zinc-300 text-sm font-semibold hover:bg-zinc-800 hover:text-white transition-colors flex items-center justify-center gap-2"
                >
        ...
        <header className="hidden lg:flex h-16 items-center justify-between px-6 border-b border-border bg-background z-30 flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-semibold text-foreground line-clamp-1 max-w-md">
                  {currentLesson?.title || "Course Viewer"}
                </h1>
                <p className="text-xs text-muted-foreground">
                  ...
                  {purchase?.course.title} • Lesson {currentLessonIndex + 1} of{" "}
                  {lessons.length}
                </p>
              </div>
            </div>
        ...
        <div className="flex-1 flex overflow-hidden relative">
          ...
          <div className="flex-1 flex flex-col h-full min-h-0 overflow-y-auto scrollbar-thin scrollbar-thumb-border bg-background">
            ...
            <div className="sticky top-0 z-50 flex flex-col bg-background shadow-md lg:shadow-none lg:bg-transparent">
              ...
              {currentLesson?.video_url ? (
                <>
                      ...
                      <video
                        ...
                        onError={(e) => {
                          ...
                          if (
                            ...
                            console.log(
                              "🔄 Video error - trying direct URL fallback..."
                            );
                          ...
                          setIsVideoLoading(false);
                          setError(
                            `Video failed to load (Error ${err?.code}). Please refresh the page.`
                          );
                        }}
                  ...
                  {securityWarning && (
                    <div className="absolute inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300">
                      ...
                      <p className="text-zinc-400 max-w-md text-sm mb-6">
                        Please stop any screen recording software or browser
                        extensions to continue watching.
                        <br />
                  ...
                  >
                    {!isPlaying && (
                      <button
                        type="button"
                        className="pointer-events-auto w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-primary/90 flex items-center justify-center shadow-2xl scale-100 hover:scale-110 transition-transform duration-200"
                        onClick={(e) => {
                          e.stopPropagation();
                          togglePlay();
                        }}
                      ...
                      <div className="flex items-center gap-2">
                        <div className="relative">
                                ...
                                {settingsView === "quality" && (
                                  <div className="py-1">
                                    ...
                                    <p className="px-9 py-1.5 text-[9px] text-zinc-500 leading-tight">
                                      Auto quality delivers the best experience
                                      for your connection.
                                    </p>
                                  </div>
                                )}
                          ...
                          <Button
                            ...
                            size="icon"
                            className={cn(
                              "text-white/80 hover:text-white w-8 h-8 transition-transform duration-200",
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-muted-foreground bg-muted border-b border-border">
          ...
          {/* Desktop Details (Description etc) */}
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-zinc-500 text-sm">
                          {formatTime(currentLesson?.video_duration || 0)}
                        </span>
                      </div>
                    ...
                    {currentLesson?.description && (
                      <div className="bg-muted/50 rounded-xl border border-border p-6">
                        ...
                        <div className="text-foreground/80 leading-relaxed whitespace-pre-wrap text-base">
                          <div
                            className={cn(
                              "relative",
            ...
            <div className="lg:hidden p-4 space-y-6 bg-background pb-20">
                ...
                {currentLesson?.description &&
                  currentLesson.description.length > 100 && (
                    <button
                      ...
                      className="text-[10px] font-bold text-foreground mt-1 uppercase tracking-wide opacity-70 hover:opacity-100"
                    >
              ...
              {lessons.length > 0 && (
                <div>
                  <h3 className="font-bold text-sm text-foreground mb-3 px-1">
                    Up Next
                  </h3>
                  <div className="space-y-3">
                    {lessons.map((lesson, index) => {
                      ...
                      const thumbnailSrc = getLessonThumbnailSrc(lesson.id);
                      ...
                      return (
                        ...
                        >
                          <div className="relative w-28 aspect-video bg-muted rounded-lg overflow-hidden flex-shrink-0 border border-border">
                            {thumbnailSrc ? (
                              <img
                                src={thumbnailSrc}
                              ...
                              <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted">
                                ...
                                <div className="text-[9px] text-muted-foreground/70 mt-0.5 line-clamp-1 px-1 text-center">
                                  {lesson.title}
                                </div>
                              </div>
                            )}
          ...
          <aside
            className={cn(
              "hidden lg:flex flex-col w-96 border-l border-border bg-background flex-shrink-0 transition-all duration-300",
                ...
                {lessons.map((lesson, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "w-full flex gap-3 p-2 rounded-lg text-left transition-all hover:bg-muted group",
                        isCurrent && "bg-muted"
                      )}
                    >
                      <div className="relative w-32 aspect-video bg-muted rounded overflow-hidden flex-shrink-0 border border-border group-hover:border-primary/20 transition-colors">
                        {thumbnailSrc ? (
                          <img
                            src={thumbnailSrc}
                            className="absolute inset-0 w-full h-full object-cover"
                        ...
                        {lessonProgress &&
                          !isCompleted &&
                          lessonProgress.progress_seconds > 0 && (
                      ...
                      <div className="flex-1 min-w-0 py-0.5">
                        <h4
                          className={cn(
                            "text-sm font-medium line-clamp-2 leading-snug group-hover:text-primary transition-colors",
                            isCurrent ? "text-primary" : "text-foreground"
                          )}
                        >
                          {lesson.title}
