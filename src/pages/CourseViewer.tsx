...
export default function CourseViewer() {
    ...
    useEffect(() => {
        async function loadCourse() {
            ...
            try {
                ...
                const { data: result, error: fnError } = await supabase.functions.invoke('recover-course-access', {
                    body: { token: accessToken }
                });

                if (fnError || !result?.found) {
                    throw new Error(result?.error || fnError?.message || 'Invalid or expired access link');
                }
    ...
    }, [isIOS, accessToken, isPlaying, lessons, currentLesson?.id, SUPABASE_FN_URL]);
    ...
    const getLessonDurationSeconds = (lesson: LessonData) => {
        if (lesson.video_duration && lesson.video_duration > 0) return lesson.video_duration;
        if (lesson.id === currentLesson?.id && duration > 0) return Math.floor(duration);
        return 0;
    };
    ...
    const handleAccessRecovery = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { data, error: fnError } = await supabase.functions.invoke('recover-course-access', {
                body: { email: recoveryEmail.trim() },
            });

            if (fnError) throw new Error('Lookup service unavailable. Please try again.');
            ...
            if (data?.found && data.purchases?.length > 0) {
                ...
                if (purchases.length === 1) {
                    setRecoveryMessage({ type: 'success', text: `Found your purchase! Redirecting to "${latest.course_title}"...` });
                    setTimeout(() => {
                ...
                setRecoveryMessage({ type: 'error', text: 'No purchases found for this email. Please check your email address or contact support.' });
            }
        } catch (err: any) {
            setRecoveryMessage({ type: 'error', text: err.message || 'Something went wrong. Please try again.' });
        } finally {
                    ...
                    <div className="bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl">
                        ...
                        <div className="p-8">
                            ...
                            <p className="text-zinc-400 text-sm text-center mb-2">
                                Enter the email you used when purchasing this course and we'll send your access link right away.
                            </p>

                            {/* Divider */}
                            <div className="flex items-center gap-4 mb-6 mt-6">
                                <div className="flex-1 h-px bg-zinc-800" />
                                <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Get My Access</span>
                                <div className="flex-1 h-px bg-zinc-800" />
                            </div>
                            ...
                            <form onSubmit={handleAccessRecovery} className="space-y-4">
                                ...
                                >
                                    {isRecovering ? (
                                        <><Loader2 className="w-4 h-4 animate-spin mr-2" />Looking up your purchase...</>
                                    ) : (
                                        'Send My Access Link →'
                                    )}
                                </Button>
                            ...
                            <div className="mt-6 pt-6 border-t border-zinc-800">
                                <p className="text-xs text-zinc-500 text-center mb-4">Already have an account? Sign in below</p>
                                <button
                                    onClick={async () => {
    ...
    return (
        <>
            <div
                className="dark flex flex-col h-screen bg-[#0a0a0a] text-white font-sans selection:bg-primary/30 overflow-hidden select-none"
                onContextMenu={(e) => e.preventDefault()}
            >
                {/* Desktop Header - Global Full Width */}
                <header className="hidden lg:flex h-16 items-center justify-between px-6 border-b border-white/5 bg-[#0a0a0a] z-30 flex-shrink-0">
                ...
                <div className="flex-1 flex overflow-hidden relative">
                    ...
                    <div className="flex-1 flex flex-col h-full min-h-0 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-800 bg-black">
                        ...
                        <div className="lg:hidden p-4 space-y-6 bg-[#0a0a0a] pb-20">
                            <div className="flex items-center justify-between">
                                <Button
                                    variant="ghost"
                                    size="sm"
                                ...
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        {lessons.map((lesson, index) => {
                                            ...
                                            const lessonDurationSeconds = getLessonDurationSeconds(lesson);
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className="relative w-28 aspect-video bg-zinc-900 rounded-lg overflow-hidden flex-shrink-0 border border-white/5">
                                                        {thumbnailSrc ? (
                                                            <img src={thumbnailSrc} className="absolute inset-0 w-full h-full object-cover" alt="" draggable={false} />
                                                        ) : (
                    ...
                    <aside
                        className={cn(
                            'hidden lg:flex flex-col w-96 border-l border-white/5 bg-[#0a0a0a] flex-shrink-0 transition-all duration-300',
                            !sidebarOpen && 'w-0 border-l-0 opacity-0 overflow-hidden'
                        )}
                    >
                        <div className="p-4 border-b border-white/5 flex items-center justify-between bg-[#0a0a0a]/80 backdrop-blur-sm">
                            <div className="flex items-center gap-2">
